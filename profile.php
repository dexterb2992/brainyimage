...
class ProfileController
{
    ...
    public function updateUser($inputs, $update_type)
    {
    ...
    public function validate($inputs, $update_type)
    {
        $errors = array();
        foreach ($inputs as $key => $value) {
            if ($inputs[$key] == "") {
                $errors[$key][] = "This $key field is required.";
            }
        }
        ...
        switch ($update_type) {
            case 'update_password':
                ...
                if (strlen($inputs['password']) < 6) {
                    $errors['password'][] = "The password should be atleast 6 characters.";
                }
            ...
            case 'general':
                if (!Helper::checkLicenseKey($inputs['access_key'], $inputs['email'])) {
                    $errors['access_key'][] = "Sorry, we can't find the license and email that you entered on our records.";
                }
                break;
        }
